...
- `playback grant` — a bare `pipewire` or `pulseaudio`: `bubbler.audio = "playback"`; with the drop-in installed, sinks and the client's own objects stay visible and every `Audio/Source` is hidden and unlinkable.
