...
mod dri;
use dri::dri;
pub(crate) use dri::dri_binds_a_primary_node;
...
mod tests {
    ...
    pub(super) fn has_seq(argv: &[String], seq: &[&str]) -> bool {
    ...
    pub(super) fn binds(argv: &[String]) -> Vec<&str> {
        let dir = argv
            .iter()
            .position(|a| a == "--dir")
    ...
    pub(super) const GPU_A: &str = "/sys/devices/pci0000:00/0000:00:01.1/0000:01:00.0";
    pub(super) const GPU_B: &str = "/sys/devices/pci0000:00/0000:00:08.1/0000:0c:00.0";

    pub(super) fn two_gpu_host() -> Vec<(&'static str, Kind)> {
        vec![
            ("/dev/dri", Dir),
            ("/dev/dri/by-path", Dir),
    ...
    pub(super) fn two_gpu_links() -> Vec<(&'static str, &'static str)> {
        vec![
    ...
    pub(super) fn seq_at(argv: &[String], seq: &[&str]) -> Option<usize> {
        argv.windows(seq.len())
            .position(|w| w.iter().map(String::as_str).eq(seq.iter().copied()))
    }

    /// A `gamepad` grant with the two device-class properties as given.
    pub(super) fn pad(hidraw: bool, uinput: bool) -> Service {
        Service::Gamepad { hidraw, uinput }
    }

    pub(super) fn gamepad_host() -> Vec<(&'static str, Kind)> {
        vec![
            ("/dev/input", Dir),
            ("/sys/class/input", Dir),
            ("/sys/devices", Dir),
