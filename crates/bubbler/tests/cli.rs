...
fn profile_lint_all_reads_every_layer_once_and_json_carries_the_counts() {
    ...
    // Several built-in profiles grant `pulseaudio`; without this, the
    // counts below would measure whether this host's pipewire daemon
    // still loads modules on request (`pulseaudio-module-loading`) rather
    // than the profiles' own grants.
...
/// `steam` skips instead of failing: `try`'s empty home makes every run
...
mod profile_smoke {
    ...
    fn spotify_window_appears() {
        gui_smoke(GuiProfile {
            ...
            // Lower case: the client is an X11 application on a session
            // that has a display for it, and the sandbox gives it none,
            // so it takes the Wayland path and a Wayland app id.
            class: "spotify",
            home_dirs: &[],
            network: true,
            x11_host: false,
